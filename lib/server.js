...
Server.prototype.addDevice = function(device, fhem) {
  ...
  if( device.alexaRoom ) {
    device.alexaRoom = device.alexaRoom.toLowerCase();
      ...
      if( fhem.alexaTypes ) {
        ...
        for( var type of fhem.alexaTypes.split(/ |\n/) ) {
          ...
          if( match.length < 4 || !match[3] ) {
            this.log.error( '  wrong syntax: ' + type );
            continue;
          }
          var name = match[1];
...
log.info( intent_name );
      ...
      for( var fhem of this.connections ) {
        ...
        if( fhem.fhemIntents[intent_name] !== undefined ) {
          match = true;
          fhem.execute( 'setreading '+ fhem.alexa_device.Name +' fhemIntent '+ fhem.fhemIntents[intent_name] );
        }
      }
      if( match ) {
      ...
      var command;
      if( saved_command && intent_name == 'RoomAnswerIntent' ) {
        intent_name = saved_command.intent_name;
...
log.debug( 'value: ' + command.value );
      ...
      if( !command.room && event.session.application !== undefined )
...
log.info( intent_name +' -> '+ characteristic );
          ...
          } else if( device && device.fhem.alexaMapping && device.fhem.alexaMapping[characteristic] ) {
...
log.debug( command.value );
        ...
        if( command.room && response.response.outputSpeech.text === '' )
          ...
          if( fhem && fhem.prefixOfIntent[intent_name] !== undefined )
            response.response.outputSpeech.text += ' '+ fhem.prefixOfIntent[intent_name];
          response.response.outputSpeech.text += ' '+ command.value;
          if( fhem && fhem.suffixOfIntent[intent_name] !== undefined )
            response.response.outputSpeech.text += ' '+ fhem.suffixOfIntent[intent_name];
